...
class Payment(jc_base.Bill):
    _name = 'jc_finance.payment'
    _description = u'财务：付款单'
    _order = 'id desc'
    ...
    type_id = fields.Many2one('archives.common_archive', string=u'付款类型', domain=[('archive_name', '=', 25)])

    detail = fields.One2many('jc_finance.payment_detail', 'payment_id',
    ...
    def default_get(self, fields_):
        res = super(Payment, self).default_get(fields_)
        need_set_fields = ['customer_id', 'company_id', 'staff_id', 'department_id']
