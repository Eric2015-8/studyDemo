...
class InBill(jc_base.Bill):
    _name = 'jc_finance.in_bill'
    _description = u'财务：转入账单'
    _order = 'id desc'
    ...
    detail = fields.One2many('jc_finance.in_bill_detail', 'in_bill_id', string=u'转入账单明细', copy=True)
    ...
    def default_get(self, fields_):
        res = super(InBill, self).default_get(fields_)
