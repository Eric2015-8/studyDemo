...
class CreateOrderWizard1(models.TransientModel):
    """手机生成订单 向导 """
    _name = "jc_sale.create.order.wizard1"
    _description = u'手机创建订单的向导'
    ...
    wizard_detail = fields.One2many('jc_sale.create.order.detail.wizard2', 'detail_id', string=u'明细', copy=True)
    ...
    def _add_detail(self):
        ...
        self.env['jc_sale.create.order.detail.wizard2'].create(values)
...
class CreateOrderWizard2(models.TransientModel):
    """手机生成订单 向导-明细 """
    _name = "jc_sale.create.order.detail.wizard2"
    _description = u'手机创建订单的向导-明细'

    detail_id = fields.Many2one('jc_sale.create.order.wizard1', string='引用', required=True,
